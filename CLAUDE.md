...
**AI seam** (`src/server/lib/ai.ts` + `agent.ts`): dependency resolution is `AI_STUB=1` → forced stub / `OPENAI_API_KEY` → OpenAI Chat Completions REST (default model `gpt-5.6`, 30s chat / 90s heavy timeouts) / no key → deterministic stub. `agent.ts` hosts the 5 agent roles (assessment eval, requirement chat, QA generation, report generation, report Q&A) with the contract that **no role ever throws for AI reasons** — timeout, transport error, or schema-invalid output all degrade to the stubs in `stubs.ts` (deterministic: no Date / Math.random). JSON roles get one corrective retry; the requirement chat runs `guardVerbatimLeak` against `hiddenSpecMd` + `personaBrief`. Never send `temperature` to the gpt-5 family.
