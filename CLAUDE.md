...
**Auth** (Better Auth): owns its own routes (`/api/auth/*`), tables (`user`/`session`/`account`/`verification`), and React client (`better-auth/react`) — it lives **outside** the shared Zod/hc contract and is mounted as a statement after `AppType` is captured. `createAuth(env)` is built **per request** (D1 binding is request-scoped). Signup requires **email OTP verification** (`emailOTP` plugin + `requireEmailVerification`): sign-up returns no session (`token:null`), `/email-otp/verify-email` sets the cookie (`autoSignInAfterVerification`), unverified sign-in gets 403 `EMAIL_NOT_VERIFIED` (the login page then offers the OTP step), and duplicate-email sign-up returns an anti-enumeration generic 200. The **email seam** (`lib/email.ts`, mirrors `ai.ts`): `EMAIL_STUB=1` → forced stub / `RESEND_API_KEY` → Resend REST / no key → stub that logs the OTP (`[email-stub] …`) with the fixed code `EMAIL_STUB_OTP` (shared constant); `createAuth` **throws on a non-local `BETTER_AUTH_URL` without a real sender**, so the fixed code can never reach production. OTPs live in the existing `verification` table — no extra migration. Challenges list/detail are public; **every `/api/attempts/*` route requires a session** (401 in routes). Other users' attempts surface as 404 (`ATTEMPT_NOT_FOUND`), never 403 — existence is hidden.
...
- **workers** (`tests/unit/`, `tests/integration/`): real miniflare D1 + R2 (`env.SUBMISSIONS`), `app.fetch(req, env)` directly. `AI_STUB=1` and `EMAIL_STUB=1` are baked into `vitest.workers.config.ts` bindings — deterministic and offline, and it must stay that way. No MSW. Storage is NOT rolled back between tests in the same file — depend only on your own test's data.
