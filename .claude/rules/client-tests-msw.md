...
- `tests/mocks/` is the single mock source for jsdom tests. `factories.ts` (`buildNote`) and `handlers.ts` run results through `*Schema.parse(...)`, so a fixture that drifts from the shared schema fails at construction/response time.
